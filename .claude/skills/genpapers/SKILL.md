...
1. **S1 QC:** before dispatching corpus-hunter, run `python3 scripts/build_used_index.py` (blacklist `used-sources.txt` + `used-index.yaml`) then `python3 scripts/build_pool_index.py` (pool catalogue + coverage); pass the blacklist + `pool_select` to every S1 prompt. Slots fill from the pool first; live fetches must over-fetch and bank leftovers via `pool_add.py`. Then: every text slot has an accepted candidate (genre, CEFR verdict, length band), and **no accepted URL appears in `used-sources.txt`** (reject + re-hunt if it does). Reject skimpy metadata.
