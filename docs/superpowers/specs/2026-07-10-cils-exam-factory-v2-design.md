...
- 允许的写作留白页必须显式标记。
- per-level Pages 输出通过 accessibility、responsive 和 link 检查。稳定的 per-level HTML/quality/source pages 不显示可变 lifecycle status，只显示已绑定的 gate results，因此 promote 前后内容相同。
...
6. Sources/Quality：publisher、title、URL、accessed、adaptation note 与 gate badges；不公开 raw text，也不嵌入 draft/published 这种可变 lifecycle label。
