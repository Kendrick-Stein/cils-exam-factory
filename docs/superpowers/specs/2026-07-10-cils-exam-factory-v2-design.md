...
`paperctl snapshot` 生成规范化 artifact inventory，禁止 auditor 自己选择要 hash 的文件。inventory 使用以下摘要：

1. `source_intake_digest`：S1 获取的原始快照 hashes、来源 identity、rights 与 provisional CEFR metadata。
2. `content_digest`：最终适配文本、题目、答案及其内容约束。
3. `policy_digest`：session 开始时冻结的一份完整版本清单，包含 canonical skill、所有 role prompts、gate configs 和允许的 tool/renderer versions。所有 gate 引用同一个 `policy_digest`；单个 gate prompt 只是该清单中的一个条目，不能改变共同 `content_digest`。
4. `student_digest`：交给 blind-solver 的精确 student projection bytes。
5. `render_digest`：`content_digest + policy_digest` 加 renderer、CSS、font 和 browser/runtime version。
6. `level_release_digest`：单个 level 的 booklet、answer sheet、teacher guide、accessible HTML、sources/quality page 等稳定输出的相对路径、大小和 SHA-256。
7. `site_closure_digest`：CI 在 promote 后组装的整站文件闭包，包括 sealed v2 levels、legacy registry outputs、index 和 assets。
...
Phase 1 的 `policy_digest` 单独覆盖所有参与角色与 gate prompts 的冻结版本。manifest、ledger 和 gate reports 不进入 `content_digest`，避免循环 hash；它们必须引用 inventory digests，自身另有 report digest。
...
G1a provisional intake report 记录 `source_intake_digest + policy_digest`；G1b final fidelity report 记录 `source_intake_digest + content_digest + policy_digest`。G2/G4a reports 记录 `content_digest + policy_digest`；G3/G4b judgment reports 还记录其独立 executor prompt 在 policy inventory 中的 digest；G3 额外记录 `student_digest`。G5 记录 `render_digest + level_release_digest`。CI 部署 closure 记录 `site_closure_digest`。

修改来源、内容、blind projection、冻结 policy、renderer、level 输出或最终站点中的任意一个字节，都会使对应后续门禁失效。
...
| S1 | source-hunter 收集；source auditor 执行 G1a provisional intake | source records、私有 cache、`source_intake_digest`、provisional report |
| S2 | item-writer 写 paper model；source fidelity reviewer 执行 G1b；确定性 content renderer 执行 G2 并生成 student projection | structured texts/items/answers、final fidelity report、student projection |
...
- 首次执行 G1b/G2（T3）后，T3–T5 的任一失败都会进入内容修复预算；每个 level 总共最多两轮，source replacement、item、difficulty 和 session-local layout hint 共用这两轮，不按失败类别分别重置。
...
| 时点 | 事件 | 新建/验证的摘要与报告 |
|---|---|---|
| T0 | S0 scaffold | 冻结 `policy_digest`，创建 draft ledger |
| T1 | S1 intake | `source_intake_digest`、G1a report |
| T2 | S2 authoring | `content_digest` |
| T3 | G1b + G2 | final fidelity/contract reports；生成 `student_digest` |
| T4 | G3 + G4a/G4b | blind、adversarial、deterministic stats、item-quality reports |
| T5 | S6 pre-release render | `render_digest`、`level_release_digest`、G5 report |
| T6 | status | 只计算 `publishable: true`，不写 manifest |
| T7 | promote | 写 `status: published` 与 seal，固定 T0–T5 全部 digests |
| T8 | PR CI/deploy | 重算确定性摘要、验证 judgment bindings、重建相同 level outputs、生成 `site_closure_digest` |

修复路径：

1. T3–T5 任一失败，先写失败 event 和 `repair_round = 1|2`。
2. 修改来源或内容会产生新的 `source_intake_digest`（若来源变化）和/或 `content_digest`；旧 G1b–G5 reports、student/render/level-release digests 全部自动失效。
3. 从受影响的最早 gate 重跑，但最终必须重新经过 G1b、G2、student projection、G3、G4 和 G5，才能再次计算 publishable。
4. 两轮后仍失败或发现共享基础设施缺陷，保持 draft 并退出；不得产生 seal。
...
- `promote` 只接受 `status: draft`，且 G1b/G2/G3/G4/G5 引用同一 `content_digest + policy_digest`、G1a/G1b 引用同一 `source_intake_digest + policy_digest`、G3 的 `student_digest` 与 G5 的 `render_digest/level_release_digest` 都可由这些 inventories 派生的 level；成功后写 `status: published` 与 seal digest。
...
- **G1a provisional intake（S1）**：检查来源字段、fetch 证据、rights note、原始正文充分性、provisional CEFR/genre、同 session 跨 level 去重，以及 raw cache/`reference/` 排除规则；绑定 `source_intake_digest + policy_digest`。
- **G1b final fidelity（S2 后）**：在最终适配文本和 `content_digest` 存在后，独立 source fidelity reviewer 比较原始快照与适配文本，验证事实、体裁声音、改写幅度和最终 CEFR；绑定 `source_intake_digest + content_digest + policy_digest`。

只有 G1a 与 G1b 都 PASS，G1 才 PASS。G1 不依赖 `student_digest`；blind projection 在 G1b 和 G2 通过后生成。
...
G4 也分成确定性和 judgment 两部分：

- **G4a deterministic**：验证 `exam.yaml` 的 level/variant profile 已声明认知配额；检查 item cognition tags 是否满足配额、答案位置分布、选项数量/长度区间、词性声明、normalized duplicate options、重复题、配置化 forbidden clue patterns 和 evidence span 完整性。
- **G4b independent judgment**：独立 item-quality reviewer 验证 cognition tags 是否真实、options 是否语义与语法平行、distractor 是否可定位地错误、是否存在跨题泄漏，以及 B2/C1 的 integrate/infer/purpose-frame 项是否真的需要相应认知操作。它不能用题干关键词计数替代判断。

G4a 与 G4b 都 PASS 才算 G4 PASS。CI 只重跑 G4a，并验证 G4b report 与 `content_digest + policy_digest` 的绑定，不在 CI 中调用模型。

“difficulty target” 只是基于语言与认知特征的校准代理；没有真实考生数据时不得声称具有心理测量学难度。

### G5 Render and Release

- G5 首次在 `status: draft` 时对预发布 per-level render 执行；报告绑定 `render_digest + level_release_digest`。G0–G5 全 PASS 后才产生 `publishable: true` 计算结果。
- A4、页码、level/session 页脚、无裁切、无表格断裂、无意外空白页。
- 写作横线、字符格、OMR 圈和改写空间容量与题型/最大词数匹配。
- 允许的写作留白页必须显式标记。
- per-level Pages 输出通过 accessibility、responsive 和 link 检查。稳定的 per-level HTML/quality/source pages 不显示可变 lifecycle status；quality page 只显示 G0–G4 report IDs/digests，不嵌入正在计算的 G5 report 或 digest，因此不存在自引用，且 promote 前后内容相同。
- 首页、session index、全站导航和“published”状态只在 promote 后由 CI 根据 sealed manifests 组装，不进入预发布 `level_release_digest`。
- `promote` 封存后，PR CI 在临时空目录 clean rebuild，并重新执行 G5 的确定性检查；per-level HTML/MD/CSS 必须与预发布 `level_release_digest` 字节一致，PDF 必须在去除受控 metadata 后 hash 一致或页面像素 hash 一致。
- CI 随后组装 index/assets/legacy files 并计算新的 `site_closure_digest`；它是最终部署闭包，不要求与 promote 前不存在的整站 index 比较。
...
- 专门 fixture 验证 draft candidate pre-render 与 promoted clean rebuild 的 per-level `level_release_digest` 相同；同时验证 promote 后生成的 index/status 页面只进入 `site_closure_digest`，不会污染 per-level digest。
...
2. Session 页：五个 sealed level 的 published 状态和质量摘要；该页只在 promote 后组装。
3. Level 页：打开 HTML、下载三类 PDF、查看来源与质量。
4. Paper HTML：无障碍阅读版。
5. Answers：主动揭示，避免误点泄题。
6. Sources/Quality：publisher、title、URL、accessed、adaptation note 与 G0–G4 gate badges；不公开 raw text，也不嵌入 draft/published 或当前 G5 digest。G5 PASS 只在 promote 后组装的 session/index 状态区展示。
...
3. 对 sealed v2 levels 重跑确定性的 G0、G2、G4a；重新计算 `source_intake_digest`、`content_digest`、`policy_digest`、`student_digest` 和 `render_digest`。
4. 验证已提交的 G1a/G1b、G3 和 G4b judgment reports 的 schema、executor metadata、report digest 和 artifact bindings；CI 不重新抓取 raw source，也不重新调用 LLM。
5. 从空目录重建 v2 per-level HTML/PDF，比较 `level_release_digest` 并重跑确定性 G5；同时按 legacy registry 验证并复制固定历史文件。
6. promote 后组装 index/assets，计算 `site_closure_digest`；检查部署闭包恰好等于 `sealed v2 outputs + legacy registry outputs + generated index/assets`，额外或缺失文件均失败。
...
`legacy-markdown-v2` 只用于完整性迁移测试、synthetic schema-v2 fixtures 和对现有 Markdown package 的 hash-bound 验证；它不把历史 published sessions 升级为 v2，也不能用于生成首个新 session。Phase 2 完成后才允许用 `paper-model-v2` 创建新的 v2 session。
...
| R2 | 1 | G0/G2/G5 | source/policy/content/student/render/level-release/site-closure inventories | content 与 output tamper tests | `paperctl snapshot/audit/promote/build` |
...
| R5 | 2 | G1a/G1b | source records、intake/fidelity reports、private-cache hashes | stub/insufficient/fact-drift fixtures | `paperctl audit` |
...
| R8 | 3 | G5 | per-level Pages、sources/quality pages、site closure | a11y/responsive/link/closure tests | `paperctl build` |
