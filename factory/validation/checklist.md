...
9b. **Oral prove**: argomenti/domande self-contained and text-only (no image tracce, no insider knowledge), doable with level grammar/lexis; per-prova duration printed; the chiavi carry a memorizable model for EVERY argomento/domanda within the spoken-length range, never printed in the student copy.
