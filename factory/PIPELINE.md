...
- **Refresh registries first:** `python3 scripts/build_used_index.py` (consumed blacklist `used-sources.txt`) then `python3 scripts/build_pool_index.py` (available pool catalogue `pool-index.yaml` + coverage). Both replace the old hand-seeded `papers/<date>/used-sources.txt`.
- **Input:** the level's `testo` slots from `exam.yaml`; `factory/corpus/sources.yaml`; `factory/corpus/cefr-criteria.md`; **`factory/corpus/used-sources.txt`** (never reuse a URL on it — cross-date de-dup); **`factory/corpus/pool-index.yaml`** (the standing pool of graded texts).
- **Pool-first:** for each slot, `python3 scripts/pool_select.py --level <L> --genre <g> --words <min>-<max>`; use a returned pooled text with zero fetching. Only unfilled slots go to a live fetch.
- **Work (live fallback):** fetch up to 2 candidates (over-fetching 1–2 neighbouring levels/genres), clean to plain prose (drop nav/ads/captions), record `{url, title, publisher, published?, accessed}`, CEFR-grade (cite evidence; assign primary + `usable_levels`), and **bank every graded text** via `python3 scripts/pool_add.py`. Reject any candidate whose URL is already in `used-sources.txt`.
