...
| `factory/corpus/used-index.yaml`, `used-sources.txt` | Auto-built source registry (rich index + flat cross-date blacklist) — regenerate, never hand-edit |
...
- After generation: run `python3 scripts/format_audit.py --session <date> --levels <levels> --report papers/<date>/format-audit.json --write-manifest`, then `python3 scripts/paper_quality_audit.py --session <date> --levels <levels> --report papers/<date>/quality-audit.json --write-manifest`, then `python3 scripts/build_site.py`, commit `papers/` + `docs/` together, push, **then merge the working branch into `main` and push `main`** — Pages deploys from `main`'s `docs/`, so the site only updates when `main` moves. Publishing is automatic once all audits pass (user directive 2026-07-18); don't ask for confirmation.
