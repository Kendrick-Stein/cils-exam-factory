...
cd "$CD"
if [ "$PR" = "ALL" ]; then
  JSON=$(python3 scripts/blind_validation.py prepare --paper-dir "papers/2026-07-29/$LV")
else
  JSON=$(python3 scripts/blind_validation.py prepare --paper-dir "papers/2026-07-29/$LV" --prova "$PR")
fi
